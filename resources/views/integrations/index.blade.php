                ...
                <div class="card-body d-flex flex-column gap-4">
                    <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-start gap-3">
                        ...
                        <div class="d-flex flex-column align-items-lg-end gap-2">
                            <span class="badge badge-phoenix {{ $auth ? 'badge-phoenix-success' : 'badge-phoenix-warning' }}">
                    ...
                    <div class="row g-3">
                        <div class="col-lg-6">
                            <div class="rounded-3 border border-translucent p-3 h-100">
                                <div class="small text-uppercase fw-semibold text-body-tertiary mb-2">{{ __('Owner connection') }}</div>
                                @if ($auth)
                        ...
                        <div class="col-lg-6">
                            <div class="rounded-3 border border-translucent p-3 h-100">
                                <div class="small text-uppercase fw-semibold text-body-tertiary mb-2">{{ __('Owner-only controls') }}</div>
                                ...
                                @if ($canManage && $auth)
                                    <form method="POST"
                                        action="{{ route('integrations.disconnect', ['provider' => $provider->value], false) }}">
                                        @csrf
                                        <button type="submit" class="btn btn-phoenix-secondary">
                    ...
                    <div>
                        <div class="small text-uppercase fw-semibold text-body-tertiary mb-2">{{ __('Dependent widgets') }}</div>
                        @if ($widgets->isEmpty())
                            <div class="text-body-secondary small">{{ __('No widgets depend on this provider yet.') }}</div>
                        @else
                            <div class="row g-3">
                                @foreach ($widgets as $widget)
                                    <div class="col-xl-6">
                                        <a href="{{ route('widgets.show', $widget, false) }}"
