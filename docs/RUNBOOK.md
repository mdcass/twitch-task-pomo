...
- Shared modal UI now routes through one product-owned `x-modal` shell plus a shared `App\Livewire\Modal` host for modal-mounted Livewire children or Blade partials. The shell owns accessibility, focus, and dismissal behavior; callers own header, body, footer, and destructive markup composition; the older `x-dialog-modal` and `x-confirmation-modal` wrappers have been removed.
