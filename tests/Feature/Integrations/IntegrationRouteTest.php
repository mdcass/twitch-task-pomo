...
class IntegrationRouteTest extends TestCase
{
    ...
    public function test_integration_routes_smoke_connect_callback_and_disconnect(): void
    {
        $owner = User::factory()->withStreamerTeam()->create();
        ...
        $this->actingAs($owner)
            ->post(route('integrations.disconnect', ['provider' => 'spotify'], false))
            ->assertRedirect(route('integrations.index', absolute: false))
            ->assertSessionHas('status', 'Spotify disconnected.');
    ...
    public function test_integration_routes_reject_unsupported_providers(): void
    {
        ...
        $this->actingAs($owner)
            ->post('/integrations/youtube/disconnect')
            ->assertNotFound();
    }
